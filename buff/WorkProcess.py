...
WorkProcess sub — 1PC/2PC 통신
  실행: python "WorkProcess sub.py"      → 2PC (기본, INI 읽어서 메이플에 키 전송)
        python "WorkProcess sub.py" 1   → 1PC (F1/F2/F3 누르면 INI에만 기록)
        python "WorkProcess sub.py" 2   → 2PC (INI 읽어서 메이플에 키 전송)
  F1: 파티초대, F2: 버프사용, F3: 파티강퇴
  1PC 기본 INI: D:\\shere\\sub_command.ini
  2PC 기본 INI: \\\\DESKTOP-KIAEFEG\\shere\\sub_command.ini
"""
import os
import sys
import threading
...
# 기본 명령 파일 경로 (1PC·2PC 각각 바라볼 경로)
DEFAULT_INI_1PC = os.path.join(r"D:\shere", "sub_command.ini")
DEFAULT_INI_2PC = os.path.normpath(r"\\DESKTOP-KIAEFEG\shere\sub_command.ini")
...
def _ensure_ini_dir(path: str):
    """INI 파일 경로의 상위 폴더가 없으면 생성. 실패 시 예외 발생."""
    ...
    try:
        ...
        if e.errno == 2:
            raise OSError(e.errno, f"폴더를 만들 수 없습니다: {d} (드라이브가 없거나 권한을 확인하세요)")
        raise
    return path
...
def write_command(ini_path: str, key: str):
    ...
    if key not in ALLOWED_KEYS:
        return False
    try:
        ini_path = _ensure_ini_dir(ini_path)
...
# ---------- 1PC: 파티초대 창 OCR 후 수락 클릭 스레드 (기능테스트용 잠시 비활성) ----------
...
def start_1pc():
    ...
    if mode_1pc_active:
        ...
        return
    # 파일 없으면 생성 (폴더 없으면 먼저 생성)
    path = os.path.normpath(os.path.abspath(path))
    if not os.path.isfile(path):
        try:
            path = _ensure_ini_dir(path)
            cfg = configparser.ConfigParser()
            cfg.add_section("Commands")
    ...
    try:
        ...
        # 파티초대 수락 스레드 시작 (OCR+클릭) — 기능테스트용 잠시 비활성
...
def stop_1pc():
    ...
    if not mode_1pc_active:
        return
    # party_accept_stop_event.set()  # OCR 수락 스레드 비활성화 시 주석
    try:
...
def build_gui_1pc():
    ...
    ini_path_var = tk.StringVar(value=DEFAULT_INI_1PC)
    ...
    def _on_fkey(event):
        key = event.keysym
        if mode_1pc_active and key in ALLOWED_KEYS:
            _on_1pc_key(key)
            return "break"

    for k in ("F1", "F2", "F3"):
        root.bind(f"<{k}>", _on_fkey)
...
def build_gui_2pc():
    ...
    row1.pack(fill=tk.X, pady=4)
    ttk.Label(row1, text="명령파일(INI):").pack(side=tk.LEFT, padx=(0, 4))
    ini_path_var = tk.StringVar(value=DEFAULT_INI_2PC)
    entry = ttk.Entry(row1, textvariable=ini_path_var, width=36)
    entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
